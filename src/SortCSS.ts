...
export default function SortCSS<O extends ConfigOutput>(
	...
	outputConfig: O = 'string' as O,
	options: ConfigOptions = {
		sortMethod: SortMethod.ByCategory,
		addCategoryComments: false,
	}
): O extends 'string' | ConfigOutputString ? string : void {
	let input: string;
	if (typeof inputConfig === 'string') {
		input = inputConfig;
	} else if (inputConfig.type === 'string') {
		input = inputConfig.data;
	} else {
		input = fs.readFileSync(inputConfig.path).toString();
	}

	if (!options.sortMethod) {
		options.sortMethod = SortMethod.ByCategory;
	}
	if (!options.addCategoryComments) {
		options.addCategoryComments = false;
	}
				...
				if (a.type === 'declaration' && b.type === 'declaration') {
					if (options.sortMethod === SortMethod.ByCategory) {
						...
						if (catA === catB) {
							...
							return (
								CSS_CATEGORIES[catA].indexOf(a.property ?? '') -
								CSS_CATEGORIES[catB].indexOf(b.property ?? '')
							);
						} else {
							return position(catA) - position(catB);
						}
					} else {
						return a.property?.localeCompare(b.property ?? '') as number;
					}
				}
				return 0;
			});
	...
	if (outputConfig === 'string' || outputConfig.type === 'string') {
		return str as any;
	} else {
		fs.writeFileSync(outputConfig.path, str);
